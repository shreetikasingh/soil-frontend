...
const Navbar = ({ navigateTo, currentPage }) => {
  ...
  const isActive = (page) => currentPage === page ? 'text-emerald-700' : 'text-slate-600';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          >
            ...
            <span className="text-2xl font-bold text-emerald-900 serif tracking-tight">TerraGrow</span>
          </div>
          ...
          <div className="hidden md:flex space-x-8">
            ...
            <button 
              ...
              className={`nav-link ${isActive('predictor')} hover:text-emerald-700 font-medium transition`}
            >
              Get Recommendation
            </button>
            <button 
          ...
          >
            <i data-lucide={mobileMenuOpen ? "x" : "menu"} className="w-6 h-6 text-emerald-800"></i>
          </button>
        </div>
      </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden glass border-t border-emerald-100">
          <div className="px-4 pt-2 pb-4 space-y-2">
            ...
            <button 
              onClick={() => { navigateTo('predictor'); setMobileMenuOpen(false); }}
              className="block w-full text-left px-3 py-2 text-slate-600 hover:bg-emerald-50 rounded"
            >
              Get Recommendation
            </button>
